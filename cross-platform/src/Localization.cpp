...
const QHash<QString, QHash<QString, QString>>& table() {
    static const QHash<QString, QHash<QString, QString>> t = {
        ...
        {QStringLiteral("settings.fullNoSelection"),
         {{QStringLiteral("en"), QStringLiteral("Copy or save the whole screen when nothing is selected")},
          {QStringLiteral("ru"), QStringLiteral("Копировать или сохранять весь экран, если ничего не выделено")},
          {QStringLiteral("uk"), QStringLiteral("Копіювати або зберігати весь екран, якщо нічого не виділено")}}},
        // Kept SHORT on purpose: makeRow does not wrap its label, so a long one
        // widens the whole dialog and clips the reset column on the right.
        {QStringLiteral("settings.cache"),
         {{QStringLiteral("en"), QStringLiteral("Temporary files")},
          {QStringLiteral("ru"), QStringLiteral("Временные файлы")},
          {QStringLiteral("uk"), QStringLiteral("Тимчасові файли")}}},
        {QStringLiteral("settings.cache.auto"),
         {{QStringLiteral("en"), QStringLiteral("Delete automatically")},
          {QStringLiteral("ru"), QStringLiteral("Удалять автоматически")},
          {QStringLiteral("uk"), QStringLiteral("Видаляти автоматично")}}},
        {QStringLiteral("settings.cache.auto.never"),
         {{QStringLiteral("en"), QStringLiteral("Never")},
          {QStringLiteral("ru"), QStringLiteral("Никогда")},
          {QStringLiteral("uk"), QStringLiteral("Ніколи")}}},
        {QStringLiteral("settings.cache.auto.week"),
         {{QStringLiteral("en"), QStringLiteral("Older than a week")},
          {QStringLiteral("ru"), QStringLiteral("Старше недели")},
          {QStringLiteral("uk"), QStringLiteral("Старші за тиждень")}}},
        {QStringLiteral("settings.cache.auto.month"),
         {{QStringLiteral("en"), QStringLiteral("Older than a month")},
          {QStringLiteral("ru"), QStringLiteral("Старше месяца")},
          {QStringLiteral("uk"), QStringLiteral("Старші за місяць")}}},
        {QStringLiteral("settings.cache.clear"),
         {{QStringLiteral("en"), QStringLiteral("Clear (%1)")},
          {QStringLiteral("ru"), QStringLiteral("Очистить (%1)")},
          {QStringLiteral("uk"), QStringLiteral("Очистити (%1)")}}},
        {QStringLiteral("settings.cache.empty"),
         {{QStringLiteral("en"), QStringLiteral("Nothing to clear")},
          {QStringLiteral("ru"), QStringLiteral("Нечего очищать")},
          {QStringLiteral("uk"), QStringLiteral("Нічого очищати")}}},
        {QStringLiteral("settings.cache.failedTitle"),
         {{QStringLiteral("en"), QStringLiteral("Couldn't clear everything")},
          {QStringLiteral("ru"), QStringLiteral("Не удалось очистить всё")},
          {QStringLiteral("uk"), QStringLiteral("Не вдалося очистити все")}}},
        {QStringLiteral("settings.cache.failedBody"),
         {{QStringLiteral("en"), QStringLiteral("Some files are still in use — most likely an update that is still downloading. Try again once it has finished.")},
          {QStringLiteral("ru"), QStringLiteral("Часть файлов занята — скорее всего, обновление ещё скачивается. Повтори, когда оно закончится.")},
          {QStringLiteral("uk"), QStringLiteral("Частина файлів зайнята — найімовірніше, оновлення ще завантажується. Повтори, коли воно завершиться.")}}},
