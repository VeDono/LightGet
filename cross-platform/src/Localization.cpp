...
const QHash<QString, QHash<QString, QString>>& table() {
    static const QHash<QString, QHash<QString, QString>> t = {
        ...
        {QStringLiteral("update.title"),
         {{QStringLiteral("en"), QStringLiteral("Software update")},
          {QStringLiteral("ru"), QStringLiteral("Обновление программы")},
          {QStringLiteral("uk"), QStringLiteral("Оновлення програми")}}},
        {QStringLiteral("update.available"),
         {{QStringLiteral("en"), QStringLiteral("LightGet %1 is available — you have %2.")},
          {QStringLiteral("ru"), QStringLiteral("Доступна версия LightGet %1 — у тебя %2.")},
          {QStringLiteral("uk"), QStringLiteral("Доступна версія LightGet %1 — у тебе %2.")}}},
        {QStringLiteral("update.upToDate"),
         {{QStringLiteral("en"), QStringLiteral("LightGet %1 is the latest version.")},
          {QStringLiteral("ru"), QStringLiteral("LightGet %1 — это последняя версия.")},
          {QStringLiteral("uk"), QStringLiteral("LightGet %1 — це остання версія.")}}},
        {QStringLiteral("update.failed"),
         {{QStringLiteral("en"), QStringLiteral("Couldn't check for updates.\n\n%1")},
          {QStringLiteral("ru"), QStringLiteral("Не удалось проверить обновления.\n\n%1")},
          {QStringLiteral("uk"), QStringLiteral("Не вдалося перевірити оновлення.\n\n%1")}}},
        {QStringLiteral("update.downloadFailed"),
         {{QStringLiteral("en"), QStringLiteral("the download didn't finish")},
          {QStringLiteral("ru"), QStringLiteral("загрузка не завершилась")},
          {QStringLiteral("uk"), QStringLiteral("завантаження не завершилося")}}},
        {QStringLiteral("update.install"),
         {{QStringLiteral("en"), QStringLiteral("Update now")},
          {QStringLiteral("ru"), QStringLiteral("Обновить")},
          {QStringLiteral("uk"), QStringLiteral("Оновити")}}},
        {QStringLiteral("update.openPage"),
         {{QStringLiteral("en"), QStringLiteral("Open download page")},
          {QStringLiteral("ru"), QStringLiteral("Открыть страницу загрузки")},
          {QStringLiteral("uk"), QStringLiteral("Відкрити сторінку завантаження")}}},
        {QStringLiteral("update.skip"),
         {{QStringLiteral("en"), QStringLiteral("Skip this version")},
          {QStringLiteral("ru"), QStringLiteral("Пропустить эту версию")},
          {QStringLiteral("uk"), QStringLiteral("Пропустити цю версію")}}},
        {QStringLiteral("update.later"),
         {{QStringLiteral("en"), QStringLiteral("Later")},
          {QStringLiteral("ru"), QStringLiteral("Позже")},
          {QStringLiteral("uk"), QStringLiteral("Пізніше")}}},
        {QStringLiteral("update.rateLimited"),
         {{QStringLiteral("en"), QStringLiteral("GitHub is temporarily refusing requests from this network. Try again in a few minutes.")},
          {QStringLiteral("ru"), QStringLiteral("GitHub временно отклоняет запросы с этой сети. Попробуй через несколько минут.")},
          {QStringLiteral("uk"), QStringLiteral("GitHub тимчасово відхиляє запити з цієї мережі. Спробуй за кілька хвилин.")}}},
        {QStringLiteral("update.cancel"),
         {{QStringLiteral("en"), QStringLiteral("Cancel")},
          {QStringLiteral("ru"), QStringLiteral("Отмена")},
          {QStringLiteral("uk"), QStringLiteral("Скасувати")}}},
        {QStringLiteral("update.downloading"),
         {{QStringLiteral("en"), QStringLiteral("Downloading the update…")},
          {QStringLiteral("ru"), QStringLiteral("Загрузка обновления…")},
          {QStringLiteral("uk"), QStringLiteral("Завантаження оновлення…")}}},
        {QStringLiteral("menu.checking"),
         {{QStringLiteral("en"), QStringLiteral("Checking for updates…")},
          {QStringLiteral("ru"), QStringLiteral("Проверяю обновления…")},
          {QStringLiteral("uk"), QStringLiteral("Перевіряю оновлення…")}}},
        {QStringLiteral("menu.checkUpdates"),
         {{QStringLiteral("en"), QStringLiteral("Check for Updates…")},
          {QStringLiteral("ru"), QStringLiteral("Проверить обновления…")},
          {QStringLiteral("uk"), QStringLiteral("Перевірити оновлення…")}}},
        {QStringLiteral("settings.autoUpdate"),
         {{QStringLiteral("en"), QStringLiteral("Check for updates on launch")},
          {QStringLiteral("ru"), QStringLiteral("Проверять обновления при запуске")},
          {QStringLiteral("uk"), QStringLiteral("Перевіряти оновлення під час запуску")}}},
        ...
        {QStringLiteral("hotkey.printscreen.title"),
         {{QStringLiteral("en"), QStringLiteral("Windows also uses Print Screen")},
          {QStringLiteral("ru"), QStringLiteral("Windows тоже использует Print Screen")},
          {QStringLiteral("uk"), QStringLiteral("Windows теж використовує Print Screen")}}},
        {QStringLiteral("hotkey.printscreen.body"),
         {{QStringLiteral("en"), QStringLiteral("Windows opens its own Snipping Tool on this key, so the two compete for every press. Turn the Windows binding off and the key is LightGet's alone.\n\nYou can restore it any time in Settings > Accessibility > Keyboard.")},
          {QStringLiteral("ru"), QStringLiteral("Windows открывает по этой клавише свой «Инструмент ножницы», поэтому вы боретесь за каждое нажатие. Если отключить системную привязку, клавиша останется только за LightGet.\n\nВернуть её можно в любой момент: Параметры > Специальные возможности > Клавиатура.")},
          {QStringLiteral("uk"), QStringLiteral("Windows відкриває цією клавішею власний «Інструмент ножиці», тож ви змагаєтесь за кожне натискання. Якщо вимкнути системну прив'язку, клавіша належатиме лише LightGet.\n\nПовернути її можна будь-коли: Параметри > Спеціальні можливості > Клавіатура.")}}},
        {QStringLiteral("hotkey.printscreen.disable"),
         {{QStringLiteral("en"), QStringLiteral("Turn it off")},
          {QStringLiteral("ru"), QStringLiteral("Отключить")},
          {QStringLiteral("uk"), QStringLiteral("Вимкнути")}}},
        {QStringLiteral("hotkey.printscreen.keep"),
         {{QStringLiteral("en"), QStringLiteral("Leave it on")},
          {QStringLiteral("ru"), QStringLiteral("Оставить")},
          {QStringLiteral("uk"), QStringLiteral("Залишити")}}},
